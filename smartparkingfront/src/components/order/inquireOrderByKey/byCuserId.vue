<template>
    <div>
        <div>
            <el-input
                class="idinput"
                placeholder="请输入常驻用户ID"
                prefix-icon="el-icon-search"
                v-model="get.cuserid">
            </el-input>
            <el-button class="idbutton" size="medium" icon="el-icon-search" type="primary" @click="initTabelData" round>查询</el-button>
        </div>
        <div class="dataMain">
            <el-table
                :data="tablelist"
...
export default {
    ...
    methods:{
        ...
        initTabelData(){
            if (this.get.cuserid){
                this.getRequest('/orderlist/getorderbyuserid/'+this.get.cuserid).then(resp=>{
                    if (resp){
                        this.tablelist = resp;
                        this.$message({
                            message: '查询成功！',
